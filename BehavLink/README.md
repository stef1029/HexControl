...
#### `wait_for_event(port, timeout, consume, auto_acknowledge, drain_first)`
...
Blocks until a sensor event is received. By default, drains any stale events first to ensure only fresh events are returned.
...
| `drain_first` | `bool` | `True` | If `True` (default), clears all buffered events before waiting. This prevents returning stale events from previous trials. Set to `False` if you need to check events that arrived while doing other work. |
