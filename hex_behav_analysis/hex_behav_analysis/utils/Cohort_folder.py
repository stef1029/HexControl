        ...
        for mouse in self.cohort["mice"]:
            for session in self.cohort["mice"][mouse]["sessions"]:
                ...
                if self.ephys_data:
                    ...
                    spikesorter_info = self.check_spikesorter_output(group_folder)
                    self.cohort["mice"][mouse]["sessions"][session]["spikesorter_output"] = spikesorter_info
                ...
                try:
                    ...
                    print(f"[ERROR] Failed to get video length for session {session} (mouse: {mouse})")
                    print(f"        Session directory: {session_folder}")
                    print(f"        Error: {str(e)}")
                    raw_data["video_length"] = None
            ...
            try:
                with open(tracker_data) as f:
                    ...
                    if "start_time" not in data or "end_time" not in data:
                        print(f"[WARNING] Missing 'start_time' or 'end_time' in tracker data: {tracker_data}")
                        return None
                ...
                print(f"[WARNING] Invalid JSON in tracker data file {tracker_data}: {str(e)}")
                return None
            except ValueError as e:
                print(f"[WARNING] Invalid time format in tracker data file {tracker_data}: {str(e)}")
                return None
            except Exception as e:
                print(f"[WARNING] Unexpected error reading tracker data file {tracker_data}: {str(e)}")
                return None
        else:
            return None
...
# Replace the existing check_spikesorter_output method in Cohort_folder class with this:

    def check_spikesorter_output(self, group_folder):
        """
        Check for spikesorter output files, specifically the phy params.py file.
        Also checks for good neurons in cluster_group.tsv.
        
        :param group_folder: Path to the group folder
        :return: Dictionary containing spikesorter output information
        """
        spikesorter_info = {
            "has_spikesorter_output": False,
            "phy_params_file": None,
            "spikesorter_path": None,
            "has_good_neurons": False  # Add this tag
        }
        
        # Expected path structure: group_folder/spikesorter_output_ephys/combined/phy_combined/params.py
        spikesorter_path = group_folder / "spikesorter_output_ephys" / "combined" / "phy_combined"
        params_file = spikesorter_path / "params.py"
        
        if params_file.exists() and params_file.is_file():
            spikesorter_info["has_spikesorter_output"] = True
            spikesorter_info["phy_params_file"] = str(params_file)
            spikesorter_info["spikesorter_path"] = str(spikesorter_path)
            
            # Check for good neurons
            cluster_group_file = spikesorter_path / "cluster_group.tsv"
            if cluster_group_file.exists():
                try:
                    import pandas as pd
                    cluster_groups = pd.read_csv(cluster_group_file, sep='\t')
                    good_units = cluster_groups[cluster_groups['group'] == 'good']
                    spikesorter_info["has_good_neurons"] = len(good_units) > 0
                except Exception as e:
                    print(f"[WARNING] Error reading cluster_group.tsv: {e}")
                    spikesorter_info["has_good_neurons"] = False
            else:
                spikesorter_info["has_good_neurons"] = False
        
        return spikesorter_info
